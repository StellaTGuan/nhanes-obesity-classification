...
dat2 <- dat_rawvars %>%
  ...
  # 4) Sanity checks (set out-of-range to NA)
  mutate(
...
# Adults 20+, non-pregnant, non-missing outcome
dat2 <- dat2 %>%
  filter(
    !is.na(bmi),
    age >= 20,
    is.na(pregnant_at_exam) | pregnant_at_exam == 0
  ) %>%
  select(-pregnant_at_exam)

# Recode educ to 3 levels
dat2 <- dat2 %>%
  mutate(
    educ = case_when(
...
# ---------------------------------------------------------
# DROP ALL NAs HERE (complete cases dataset is what you save)
# ---------------------------------------------------------
dat2 <- na.omit(dat2)

# --- Save cleaned, feature-ready dataset (NO NAs) ---
write_csv(
  dat2,
...
# BMI category distribution figure (based on NO-NA dataset)
bmi_dist <- dat2 %>%
  mutate(
    bmi_cat = case_when(
...
p <- ggplot(bmi_dist, aes(x = bmi_cat, y = n)) +
  geom_col(width = 0.35, fill = "lightblue") +
